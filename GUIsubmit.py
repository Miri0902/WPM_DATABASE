...
class UpdateExistingUsers:
    ...
    def update_existing_user(self):
        while True:
            user_id_to_update = input("Enter the UserID or Username of the user you want to update: ")

            with sqlite3.connect(self.db_filename) as conn:
                cursor = conn.cursor()

                # Check if input is a number (assume it's a UserID) or not (assume it's a Username)
                if user_id_to_update.isdigit():
                    cursor.execute("SELECT * FROM users WHERE userID = ?", (user_id_to_update,))
                else:
                    cursor.execute("SELECT * FROM users WHERE username = ?", (user_id_to_update,))

                user_data = cursor.fetchone()

                if not user_data:
                    print(f"User with UserID/Username {user_id_to_update} not found.")
                    break

                user_columns = [col[0] for col in cursor.description]
                user = dict(zip(user_columns, user_data))

                print(f"Updating UserID {user['userID']} and username {user['username']}:")
                ...
                for col in columns_to_update:
                    ...
                    cursor.execute(f"UPDATE users SET {col} = ? WHERE userID = ?",
                                   (new_value, user['userID']))
                    conn.commit()

                print(f"User details for UserID {user['userID']} and username {user['username']} updated successfully.")
